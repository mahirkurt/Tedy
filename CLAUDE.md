...
- **Multi-page routing**: `react-router-dom` with routes defined in `dashboard/src/routes.ts`. Pages: Bugün (today), Program, Ödevler, Notlar, Takvim, Takımlar, Dersler, Tedy Books, İlerleme, Duyurular, Profil. Carbon `SideNav` for navigation, built from `navRoutesFor(role)` — routes flagged `showInNav: false` (the book/reader detail routes) are routable but hidden.
