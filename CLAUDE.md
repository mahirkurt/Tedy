...
- **Asistanın hitabı**: the model is told who is asking. `roles.okur_turu()` maps the session email to `ogrenci` (Işık's own account, `OGRENCI_EMAILS`), `aile` (any other full-role member) or `bilinmiyor` (API keys, `/v1`, the test bypass); `_assistant_okur()` reads it inside the request on `/api/assistant/chat`, `/stream` and `/plan`, and the user turn carries "Soran: …" next to "Bugün: …". The prompt's "## Hitap" says "sen" to Işık, never Işık in the third person; "siz" to the family, with Işık by name. The page follows: `/api/auth/me` returns `student`, and `AssistantChat`'s `VOICE` switches the greeting, quick prompts, placeholder, source note and the asker's label (a parent's question used to be labelled "Işık"). Measured before: one answer said "önünde şu ödevler var Işık" and then "Işık zaten Yaptım demiş". Citation chips are renumbered in reading order by `_finalize_citations` — a lone chip used to read "3" because the third tool's source was the only one cited. `_finalize_citations` also drops a marker that repeats the one just before it in its section (`_tekrari_topla`; a heading or a bold-only line starts a section): an answer drawn wholly from `odev_listesi` put a "1" after every sentence.
- **How an answer is set** (2026-09-25): the prompt's `## Biçim` fixes one skeleton — a one- or two-sentence direct answer, `###` sections (2–5 words), list items as "**name** — note", at most one closing `**Şimdi:** …` (the first small step) and one `**Not:** …`, no tables, rules, quotes or emoji. `AssistantChat` renders with `renderMarkdown(text, { bicim: 'sohbet' })`: `ac-md__*` classes in Carbon type (styles in `AssistantChat.scss`), never Tedy Books' `bookmd__*` chapter styles, which had given answers serif headings, a book's first-line indent and loose-verse italics. The chat variant turns a bold-only line into a heading (h3 for sections, h4 below — the page title is h2), sets `Şimdi`/`Öneri`/`İpucu` and `Not`/`Dikkat`/`Hatırlatma` lead-ins as callouts, renders a pipe table if one slips through, and skips loose-verse folding. Lists in both variants survive a blank line between items and one level of nesting (they used to restart at 1). Pinned by `tests/e2e/asistan-cevap-bicimi.spec.ts` (with screenshot baselines).
- **Asistan ve müfredat (maarif MCP)** (measured 2026-09-25): seven tools (`kazanim_ara`, `kazanim_listele`, `mufredat_ara`, `kitap_listele`, `kitap_sayfa`, `figur_ara`, `figur_getir`) reach CureoHub's `mufredat-mcp` at `https://mufredat.cureonics.com/mcp` with `Authorization: Bearer $MUFREDAT_MCP_API_KEY` (21 tools served; facts in CureoHub `mcp-servers/mufredat-mcp/README.md`, `DISCOVERY-UPSTREAM-2026-09.md`). A search the model does not scope gets Işık's grade (`McpRegistry.sinif` ← `_mufredat_sinifi()`, "7.Sınıf" from the profile; an explicit `grade` wins, an unknown class invents none) — unscoped, an outcome search mixed every year. `pdf_url`/`source_url` are stripped from every maarif result before the model reads it: since MEB moved its files in summer 2026 all of them answer HTTP 500. A textbook page is cited by the book's title ("Matematik 6.Sınıf Ders Kitabı · s.84"), not its corpus number. **Coverage gap:** the corpus (built 2026-06-14) holds 105 textbooks; of the 19 grade-7 books MEB now lists only the two Multi English ones are in it, so for most subjects the assistant can ground on outcomes and programme text but not on the book — the prompt tells it to say so rather than offer another grade's book. Closing it is CureoHub's corpus migration (`docs/superpowers/plans/2026-09-12-mufredat-korpus-gocu.md`, not yet run). **The tool loop** counts rounds (model turns), not calls — `max_rounds=4`, `max_calls=8`; until 2026-09-25 two parallel calls spent two of four and the fifth never ran. When the budget is spent the last turn carries `SON_TUR_NOTU` ("cevabı şimdi yaz") with tools withdrawn and is asked once more if it comes back empty: withdrawing the tools alone was measured to return a thinking block and no text, and the reader got "cevap üretemedim" after four successful tool calls.
...
- **One column, audited**: every page is the same 960px column (`width: 100%`, `calc(100% - 16rem)` beside the nav). The ≥672px rule `margin: 0 auto` made the shell a shrink-to-fit flex item, so each page took its content's width — Bugün 736px, İlerleme 290px — and on phones Carbon's `margin-top: 3rem` on `.cds--content` survived, starting every page 56px low. Portal banners span the column (Carbon caps InlineNotification at 46rem). `dashboard/tests/e2e/tasarim-denetimi.spec.ts` runs axe (`@axe-core/playwright`, a dev dependency, WCAG 2.2 AA + best practice) on all 13 pages at 1440 and 390px with the `FULL` fixture and fails on any violation or sideways scroll; it also pins the equal column, the phone top offset and that opening Asistan does not scroll the window (the chat scrolls its own pane, never `scrollIntoView`). Past items recede by colour, not opacity (opacity put calendar titles at 3.03:1). The week table keeps a 40rem minimum and scrolls sideways inside Carbon's container, which is made a named, focusable region; the hour column is sticky. On the brand band focus is `focus-inverse`. Radii follow the Tedy layer: cards and panels `--ted-radius-card` (8px), controls 0 — there is no 4px. The profile scraper raises on the login page (a dropped session had turned "Beni hatırla / Remember Me: on" into the only profile field).
- **The audit suite** (`dashboard/tests/e2e/`, added 2026-09-25) — all on fixed data (`_gorsel-fixtures.ts`: the `FULL` fixture re-anchored to a school Thursday, `SABIT_SAAT` 2026-09-24 10:30 Istanbul, real-shape exams; shared opener `sabitAc` in `_gorsel-yardim.ts` fixes the clock, mocks every endpoint and waits for fonts). `gorsel-regresyon` compares 13 pages × 1440/390px plus the sign-in page pixel by pixel (`maxDiffPixelRatio` 0.002); `aria-yapisi` pins each page's accessibility tree (`.aria.yml`); `ibm-erisilebilirlik` runs IBM Equal Access (`accessibility-checker-engine`'s `ace.js` injected into the page — no network, telemetry off) and fails on violations only; it caught five things axe passed, among them a button whose `aria-label` did not contain its visible text (WCAG 2.5.3) — the health trigger is now named by its visible text plus a hidden "Senkron durumu: " prefix, so locate it by `/^Senkron durumu:/`. `gorunum-kipleri` checks that nothing animates under `prefers-reduced-motion` and that cards keep a border under `forced-colors` (both blocks at the end of `ted-theme.scss`; the white header logo swaps to the dark one through `<picture>`). `capraz-tarayici` runs in the `webkit` and `firefox` projects (`playwright.config.ts`; every other spec is Chromium only): content renders, no page error, no sideways scroll. Those browsers are not in the pinned install — `npx playwright install webkit firefox`, and if it stalls, unzip the downloads by hand and touch `INSTALLATION_COMPLETE`. `npm run lint` includes `stylelint-plugin-carbon-tokens` (`dashboard/stylelint.config.mjs`): spacing, motion and type must be Carbon tokens (`spacing.$spacing-NN`, `motion.$duration-*`, `@include type.type-style(...)` — the plugin rejects `type.type-scale()` as a value); every exception is a `stylelint-disable-next-line` with its reason, and Books is exempt as a block. Lighthouse (`lighthouserc.cjs`) serves the bypass server on 8287 and asserts accessibility ≥ 0.95, best practices ≥ 0.9, CLS ≤ 0.1; performance is a warning (~0.55 locally: 988 KB of mostly unused Carbon CSS and one 241 KiB app chunk — production is compressed by Cloudflare, local is not).
